...
const en = {
  ...
  scheduler: { running: 'Watching · running', stopped: 'Watching · stopped', loading: 'Checking…', start: '▶ Start all', stop: '■ Stop all', loginFirst: 'Log in first' },
  status: { watching: 'WATCHING', waitlisted: 'WAITLISTED', registered: 'REGISTERED', paused: 'PAUSED', stopped: 'STOPPED', error: 'ERROR' },
  card: { registerNow: '⚡ Register now', running: '… running', lastPoll: 'last poll {{rel}}', notPolled: 'not polled yet', pause: '⏸ Pause', resume: '▶ Resume', retryWatching: '⟳ Resume watching', retryHint: 'Start watching this course again after repeated failures' },
  ...
  settings: {
    ...
    pacingHint: 'How long to wait between each browser action inside a single course check, in milliseconds. This is not the poll interval above — that one decides how often a course is checked. One check performs about 9 actions, so a smaller pause finishes the whole cycle sooner. Keep it at {{min}} ms or more: a human-like pace is what keeps McGill’s servers from treating the automation as a bot. Allowed range: {{min}}–{{max}} ms.',
    pacingRange: 'Operation speed must be a number — pause {{min}}–{{max}} ms, jitter 0–{{max}} ms. Nothing was saved.',
  },
  session: {
    title: 'Session',
...
const zh: typeof en = {
  ...
  scheduler: { running: '监控 · 运行中', stopped: '监控 · 已停止', loading: '检查中…', start: '▶ 全部启动', stop: '■ 全部停止', loginFirst: '请先登录' },
  status: { watching: '监控中', waitlisted: '候补中', registered: '已注册', paused: '已暂停', stopped: '已停止', error: '错误' },
  card: { registerNow: '⚡ 立即执行', running: '… 执行中', lastPoll: '上次轮询 {{rel}}', notPolled: '尚未轮询', pause: '⏸ 暂停', resume: '▶ 恢复', retryWatching: '⟳ 重新监控', retryHint: '连续失败后重新开始监控这门课' },
  ...
  settings: {
    ...
    opPause: '操作间隔（毫秒）',
    opJitter: '操作抖动（± 毫秒）',
    pacingHint: '在一次课程查询的内部，每两个浏览器操作之间等多久，单位毫秒。这和上面的「轮询间隔」不是一回事 —— 轮询间隔决定多久查一次课程。一次查询大约有 9 个操作，间隔越小整个周期结束得越快。建议不要低于 {{min}} 毫秒：贴近真人的节奏才能避免被 McGill 服务器当成机器人。允许范围：{{min}}–{{max}} 毫秒。',
...
const fr: typeof en = {
  ...
  scheduler: { running: 'Surveillance · active', stopped: 'Surveillance · arrêtée', loading: 'Vérification…', start: '▶ Tout démarrer', stop: '■ Tout arrêter', loginFirst: "Connectez-vous d'abord" },
  ...
  card: { registerNow: '⚡ Inscrire maintenant', running: '… en cours', lastPoll: 'dernier sondage {{rel}}', notPolled: 'pas encore sondé', pause: '⏸ Pause', resume: '▶ Reprendre', retryWatching: '⟳ Relancer la surveillance', retryHint: 'Relancer la surveillance de ce cours après des échecs répétés' },
