...
describe('Scheduler.start', () => {
  ...
  it('tickSoon runs a tick only while the engine is running', async () => {
    const { scheduler, watcher, store, target } = setup({
      decision: { action: 'NOOP', reason: 'full' },
    });
...
describe('Scheduler cancels in-flight cycles on stop/pause (Q9/Q14)', () => {
  ...
  it('does not submit a registration when the scheduler is stopped mid-cycle (Stop all)', async () => {
    const { store, scheduler, actor, target, release, entered } = blockingScheduler();
    // `start()` fires one immediate tick, so it — not a hand-rolled `runOnce` — owns
    // the in-flight cycle here. On the pre-merge side `start()` only armed the timer,
    // so the test had to start the cycle itself; now the extra `runOnce` would be
    // dropped by the in-flight guard and the assertions would read a cycle that never
    // ran. Cancelling the started cycle is what this test is actually about.
    scheduler.start(); // Stop all goes through stop(); mirror that
    await entered; // start()'s immediate tick is the cycle the user is stopping
    store.updateTarget(target.id, { status: 'paused' }); // stop-all pauses first…
    scheduler.stop(); // …then stops the engine
    release();
    // The abort is logged from an unawaited cycle, so let its continuation run
  ...
  it('records the outcome when a stop lands while act() is in flight', async () => {
    ...
    // As above: `start()`'s immediate tick is the cycle whose submission we stop
    // mid-flight, so a second `runOnce` would only be dropped by the in-flight guard.
    scheduler.start();
    await enteredGate; // start()'s immediate tick is the in-flight submission
    scheduler.stop(); // stop lands mid-submission
    releaseAct();
    // Yield so the unawaited cycle records its outcome before we read the log.
    await new Promise((r) => setTimeout(r, 20));
